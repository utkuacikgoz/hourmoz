...
const CANONICAL_ORIGIN = 'https://ishormuzopen.fun';
...
for (const entry of await readdir('public', {withFileTypes: true})) {
  ...
  if (entry.name.endsWith('.html')) {
    ...
    // Canonical and social-card URLs are absolute; SITE_ORIGIN at build time repoints them for another deployment.
    if (process.env.SITE_ORIGIN) html = html.replaceAll(CANONICAL_ORIGIN, new URL(process.env.SITE_ORIGIN).origin);
