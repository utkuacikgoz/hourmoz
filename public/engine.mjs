...
export class Crossing {
 ...
 reset(mode,seed){if(seed!==undefined)this.random=seededRandom(seed);this.spawnRandom=seededRandom(this.random()*4294967296);this.pickupRandom=seededRandom(this.random()*4294967296);this.combatRandom=seededRandom(this.random()*4294967296);this.actorId=0;this.mode=mode;this.phase='play';this.time=0;this.sector=1;this.score=0;this.combo=1;this.comboTime=0;this.stopped=0;this.escaped=0;this.delivered=0;this.entities=[];this.shots=[];this.events=[];this.id=0;this.finale=false;this.exitZ=-64;this.formation=0;this.spawn=2;this.pickup=7;this.difficulty=difficultyAt(0);this.nextPost=24;this.postUntil=0;this.panicUntil=0;this.nextCheckpoint=42;this.checkpointUntil=0;this.payoffUntil=0;this.lastHit=null;this.nearMisses=0;this.player={x:0,z:23,vx:0,vz:0,hull:100,boost:100,cooldown:0,invincible:2,decoy:0};this.input={x:0,z:0,boost:false,fire:false,aim:null};}
 event(type,data={}){this.events.push({type,...data})}
 add(type,x,z,extra={}){const e={id:++this.id,type,x,z,vx:0,vz:0,age:0,hp:type==='tanker'?4:type==='escort'?3:1,r:type==='tanker'?3.4:type==='escort'?2.1:1.4,motionId:['missile','repair'].includes(type)?0:++this.actorId,cool:1+this.combatRandom()*2,...extra};this.entities.push(e);return e}
 ...
 tick(dt){if(this.phase!=='play')return;dt=Math.min(dt,.05);const p=this.player;this.time+=dt;if(this.time>=90){this.exitZ=-64+(this.time-90)*98/15;if(!this.finale){this.finale=true;this.event('finale')}}const previousStage=this.difficulty.stage;this.difficulty=difficultyAt(this.time);if(this.difficulty.stage>previousStage){this.event('difficulty',{pressure:this.difficulty.pressure,stage:this.difficulty.stage})}this.comboTime-=dt;if(this.comboTime<=0)this.combo=1;p.cooldown=Math.max(0,p.cooldown-dt);p.invincible=Math.max(0,p.invincible-dt);p.decoy=Math.max(0,p.decoy-dt);const boosting=this.input.boost&&p.boost>1;this.boosting=boosting;p.boost=clamp(p.boost+(boosting?-32:13)*dt,0,100);const speed=(this.mode==='run'?20:23)*(boosting?1.75:1);let ix=this.input.x,iz=this.input.z,len=Math.hypot(ix,iz);if(len>1){ix/=len;iz/=len}p.vx+=(ix*speed-p.vx)*Math.min(1,dt*6);p.vz+=(iz*speed-p.vz)*Math.min(1,dt*6);p.x=clamp(p.x+p.vx*dt,-30,30);p.z=clamp(p.z+p.vz*dt,-14,34);if(this.input.fire)this.ability();
 if(this.time>=this.nextPost){this.nextPost+=28;this.postUntil=this.time+2;this.panicUntil=this.time+8;this.event('post')}
 if(this.mode==='run'&&this.time>=this.nextCheckpoint){this.nextCheckpoint+=36;this.checkpointUntil=this.time+6;const gap=Math.floor(this.spawnRandom()*5);for(let lane=0;lane<5;lane++)if(lane!==gap)this.add('mine',-24+lane*12,-36,{vz:10,checkpoint:true});this.event('checkpoint')}
 ...
 if(this.spawn<=0){
  ...
  if(this.entities.length<140){const pattern=this.formation++%3;
   if(d.stage<2){const x=(this.spawnRandom()-.5)*48;this.add(this.mode==='block'?'tanker':!d.armed?'mine':'escort',x,-60,{vz:d.speed,cool:2.5});}
   else if(pattern===0&&this.mode==='run'){const gap=Math.floor(this.spawnRandom()*5);for(let lane=0;lane<5;lane++)if(Math.abs(lane-gap)>0)this.add('mine',-24+lane*12,-60,{vz:d.speed});}
 ...
 const deadShots=new Set();for(const s of this.shots){s.age+=dt;s.x+=s.vx*dt;s.z+=s.vz*dt;if(s.age>1.9){deadShots.add(s);continue}for(const e of this.entities){if(remove.has(e)||!['tanker','escort','mine'].includes(e.type))continue;if(Math.hypot((e.x-s.x)*.5,(e.z-s.z)*.75)<e.r+1){e.hp--;deadShots.add(s);this.event('impact',{x:s.x,z:s.z});if(e.hp<=0){remove.add(e);this.reward(e.type==='tanker'?250:150);this.stopped++;this.event('destroy',{x:e.x,z:e.z,big:e.type==='tanker'});if(this.combatRandom()<.2)this.add('repair',e.x,e.z,{vz:7})}break}}}
 this.entities=this.entities.filter(e=>!remove.has(e));this.shots=this.shots.filter(s=>!deadShots.has(s));if(this.phase!=='play')return;if(this.time>=105||(this.mode==='run'&&this.finale&&p.z<=this.exitZ)){if(this.mode==='run')this.delivered++;this.end(true);return}const next=1+Math.floor(this.time/35);if(next>this.sector){this.sector=next;this.delivered+=this.mode==='run'?1:0;this.score+=1000;this.entities=this.entities.filter(e=>e.z<-20);this.shots=[];this.event('sector',{sector:this.sector})}
 }
}
