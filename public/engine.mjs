...
export class Crossing {
 ...
 reset(mode){this.mode=mode;this.phase='play';this.time=0;this.sector=1;this.score=0;this.combo=1;this.comboTime=0;this.stopped=0;this.escaped=0;this.delivered=0;this.entities=[];this.shots=[];this.events=[];this.id=0;this.spawn=2;this.pickup=7;this.difficulty=difficultyAt(0);this.lastHit=null;this.nearMisses=0;this.player={x:0,z:23,vx:0,vz:0,hull:100,boost:100,cooldown:0,invincible:2,decoy:0};this.upgrades={engine:0,ability:0};this.input={x:0,z:0,boost:false,fire:false,aim:null};}
 ...
 if(this.mode==='run'&&!e.passed&&['mine','escort'].includes(e.type)&&e.z>p.z+4){e.passed=true;const gap=Math.abs(e.x-p.x);if(!e.hit&&gap>2*(e.r+1.4)&&gap<2*(e.r+3)){this.nearMisses++;this.reward(50);this.event('nearMiss')}}
