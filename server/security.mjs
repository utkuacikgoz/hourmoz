...
export async function limitRequest(request, database, secret = null) {
  ...
  if ((await count(database, `${client}:${kind}`, bucket)) > limits.client)
    throw new RequestError('Too many requests. Try again in a minute.', 429);
  if ((await count(database, `global:${kind}`, bucket)) > limits.global)
    throw new RequestError('The game is busy. Try again in a minute.', 429);
