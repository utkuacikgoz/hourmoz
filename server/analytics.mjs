...
export async function visitMetrics(db,since){
 ...
 const campaigns=await db.prepare("SELECT e.sponsor_id AS sponsor, MAX(b.name) AS name, SUM(e.viewed) AS views, SUM(e.clicked) AS clicks FROM sponsor_events e JOIN visits v ON v.id = e.visit_id LEFT JOIN sponsor_bids b ON b.id = e.sponsor_id WHERE v.created_at >= ? GROUP BY e.sponsor_id ORDER BY views DESC").bind(since).all();
 return {...totals,sponsorViews:campaigns.results.reduce((n,r)=>n+r.views,0),sponsorClicks:campaigns.results.reduce((n,r)=>n+r.clicks,0),playingVisits:rows.results.reduce((n,r)=>n+r.playingVisits,0),sources:rows.results,campaigns:campaigns.results};
}
export async function createVisit(db,data,player){
 ...
 await db.batch([db.prepare('DELETE FROM sponsor_events WHERE visit_id IN (SELECT id FROM visits WHERE created_at < ?)').bind(now-30*DAY),db.prepare('DELETE FROM visits WHERE created_at < ?').bind(now-30*DAY),db.prepare('INSERT INTO visits (id, player_id, created_at, source, sponsor_id) VALUES (?, ?, ?, ?, ?) ON CONFLICT(id) DO NOTHING').bind(data.id,player,now,sources.has(data.source)?data.source:'other',sponsor?.id??null)]);
 return {id:data.id};
}
export async function visitEvent(db,data,player){
 ...
 if(data.event==='booking_click'?!current.bookingUrl:!current.sponsor||data.sponsorId&&data.sponsorId!==current.sponsor.id)return {error:'Sponsor unavailable.',status:409};
 if(data.event!=='booking_click'){await db.prepare('INSERT INTO sponsor_events (visit_id, sponsor_id, viewed, clicked) VALUES (?, ?, ?, ?) ON CONFLICT(visit_id,sponsor_id) DO UPDATE SET viewed = MAX(viewed,excluded.viewed), clicked = MAX(clicked,excluded.clicked)').bind(visit.id,current.sponsor.id,data.event==='sponsor_view'?1:0,data.event==='sponsor_click'?1:0).run();return {ok:true}}
