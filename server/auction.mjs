...
const DAY = 86400000, OPENING_BID = 400, STEP = 100, MAX_BID = 1000000;
...
function appOrigin(env) { try { const u = new URL(env.APP_ORIGIN); if (u.protocol === 'https:' && !u.username && !u.password && u.pathname === '/' && !u.search && !u.hash) return u.origin; } catch {} fail('Payment return address is not configured.', 503); }
...
async function highBid(db, testing = 0) { return db.prepare(`SELECT id, name, url, amount FROM sponsor_bids WHERE ${ACTIVE} AND test_mode = ? ORDER BY amount DESC, paid_at ASC LIMIT 1`).bind(testing).first(); }
const minimumBid = high => (high?.amount ?? OPENING_BID) + STEP;
export async function paidSponsor(db, testing = 0) { const bid = await highBid(db, testing); return bid ? {id: bid.id, name: bid.name, url: bid.url, amount: bid.amount} : null; }
export async function publicAuction(db, enabled, testing = 0) {
  const sponsor = await paidSponsor(db, testing);
  const history = await db.prepare(`SELECT name, amount, paid_at AS paidAt FROM sponsor_bids WHERE ${ACTIVE} AND test_mode = ? ORDER BY amount DESC LIMIT 20`).bind(testing).all();
  return {enabled, testMode: !!testing, minimum: minimumBid(sponsor), highest: sponsor?.amount ?? 0, sponsor, bids: history.results};
}
...
export async function submitBid(db, env, data, player) {
  ...
  if (!bid) {
    ...
    let url;
    try { url = new URL(data.url); if (url.protocol !== 'https:' || url.username || url.password || url.href.length > 500 || !url.hostname.includes('.') || url.hostname.endsWith('.local') || /^\d+\.\d+\.\d+\.\d+$/.test(url.hostname)) throw new Error(); } catch { fail('Enter a public HTTPS website address.'); }
    ...
    if (!Number.isSafeInteger(data.amount) || data.amount < minimum || data.amount % 100 !== 0 || data.amount > MAX_BID) fail(`Bid at least $${minimum / 100} USD, in whole dollars (maximum $${MAX_BID / 100}).`, 409);
    const now = Date.now();
    const recent = await db.prepare('SELECT COUNT(*) AS count FROM sponsor_bids WHERE player_id = ? AND created_at > ?').bind(player, now - 3600000).first();
    ...
    await db.batch([
      db.prepare("DELETE FROM sponsor_bids WHERE status IN ('checkout', 'expired') AND created_at < ?").bind(now - 30 * DAY),
      db.prepare('INSERT INTO sponsor_bids (id, player_id, name, url, amount, created_at, test_mode) VALUES (?, ?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO NOTHING').bind(data.id, player, name, url.href, data.amount, now, testMode(env)),
    ]);
  ...
  if (bid.status === 'expired' || Date.now() - bid.created_at > DAY) fail('Payment attempt expired. Start a new bid.', 409);
  ...
  if (target.protocol !== 'https:' || target.hostname !== 'checkout.stripe.com') fail('Invalid checkout response.', 503);
...
export async function fulfillSession(db, env, session) {
  ...
  await db.prepare(`UPDATE sponsor_bids SET status = CASE WHEN amount > ${floor} THEN 'paid' ELSE 'refund_pending' END, paid_at = CASE WHEN amount > ${floor} THEN ? ELSE NULL END, payment_intent = ? WHERE id = ? AND status IN ('checkout', 'expired')`).bind(bid.test_mode, bid.test_mode, Date.now(), session.payment_intent, bid.id).run();
...
export async function paymentStatus(db, env, id, player) {
  ...
  if (bid.session_id && ['checkout', 'expired', 'refund_pending'].includes(bid.status)) { const session = await stripe(env, 'checkout/sessions/' + encodeURIComponent(bid.session_id)); await fulfillSession(db, env, session); }
...
export async function stripeWebhook(db, env, request) {
  ...
  while (true) { const {value, done} = await reader.read(); if (done) break; size += value.length; if (size > 65536) { await reader.cancel(); fail('Webhook too large.', 413); } chunks.push(value); }
  const bytes = new Uint8Array(size); let offset = 0;
  for (const c of chunks) { bytes.set(c, offset); offset += c.length; }
  const raw = new TextDecoder().decode(bytes);
  await verifyStripeSignature(raw, request.headers.get('Stripe-Signature'), env.STRIPE_WEBHOOK_SECRET);
  let event;
  try { event = JSON.parse(raw); } catch { fail('Invalid webhook.'); }
  const object = event?.data?.object;
  if (!object || typeof object !== 'object') return {ok: true};
  if (['checkout.session.completed', 'checkout.session.async_payment_succeeded'].includes(event.type)) return fulfillSession(db, env, object);
  if (event.type === 'checkout.session.expired' && typeof object.id === 'string') {
    await db.prepare("UPDATE sponsor_bids SET status = 'expired' WHERE session_id = ? AND status = 'checkout'").bind(object.id).run();
    ...
    await db.prepare("UPDATE sponsor_bids SET status = 'refunded' WHERE payment_intent = ? OR id = ?").bind(object.payment_intent, intent.metadata?.bid_id ?? '').run();
    ...
    if (intentId && event.type === 'charge.dispute.created') await db.prepare("UPDATE sponsor_bids SET status = 'disputed' WHERE payment_intent = ? AND status IN ('paid', 'refund_pending')").bind(intentId).run();
    else if (intentId && ['won', 'warning_closed'].includes(object.status)) await db.prepare("UPDATE sponsor_bids SET status = CASE WHEN paid_at IS NULL THEN 'refund_pending' ELSE 'paid' END WHERE payment_intent = ? AND status = 'disputed'").bind(intentId).run();
    else if (intentId) await db.prepare("UPDATE sponsor_bids SET status = 'refunded' WHERE payment_intent = ? AND status = 'disputed'").bind(intentId).run();
  }
  return {ok: true};
}
export async function ownerBids(db) {
  const rows = await db.prepare("SELECT id, name, url, amount, status, hidden, test_mode AS testMode, session_id AS sessionId, paid_at AS paidAt, created_at AS createdAt FROM sponsor_bids WHERE status != 'expired' ORDER BY CASE status WHEN 'paid' THEN 0 WHEN 'disputed' THEN 1 WHEN 'refund_pending' THEN 2 ELSE 3 END, created_at DESC LIMIT 100").all();
...
export async function reviewBid(db, data) {
  ...
  await db.prepare('UPDATE sponsor_bids SET hidden = ? WHERE id = ?').bind(data.action === 'hide' ? 1 : 0, data.id).run();
  return {ok: true};
}
