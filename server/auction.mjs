...
async function highBid(db,testing=0){return db.prepare('SELECT id, name, url, amount, status, hidden FROM sponsor_bids WHERE paid_at IS NOT NULL AND test_mode = ? ORDER BY amount DESC LIMIT 1').bind(testing).first()}
export async function paidSponsor(db,testing=0){const bid=await highBid(db,testing);return bid&&bid.status==='paid'&&!bid.hidden?{id:bid.id,name:bid.name,url:bid.url,amount:bid.amount}:null}
export async function publicAuction(db,enabled,testing=0){const high=await highBid(db,testing);const history=await db.prepare("SELECT name, amount, paid_at AS paidAt FROM sponsor_bids WHERE paid_at IS NOT NULL AND hidden = 0 AND status = 'paid' AND test_mode = ? ORDER BY amount DESC LIMIT 20").bind(testing).all();return {enabled,testMode:!!testing,minimum:(high?.amount??400)+100,highest:high?.amount??0,sponsor:await paidSponsor(db,testing),bids:history.results}}
...
export async function submitBid(db,env,data,player){
 ...
 const minimum=((await highBid(db,testMode(env)))?.amount??400)+100;
 ...
 await db.prepare('INSERT INTO sponsor_bids (id, player_id, name, url, amount, created_at, test_mode) VALUES (?, ?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO NOTHING').bind(data.id,player,name,url.href,data.amount,Date.now(),testMode(env)).run();
...
export async function fulfillSession(db,env,session){
 ...
 const expectedLive=!bid.test_mode;if(session.livemode!==expectedLive||bid.test_mode!==testMode(env))fail('Payment environment mismatch.',400);
 // Atomic comparison: a delayed or equal-price checkout can never replace a higher sponsor.
 await db.prepare("UPDATE sponsor_bids SET status = CASE WHEN amount > COALESCE((SELECT MAX(amount) FROM sponsor_bids WHERE paid_at IS NOT NULL AND test_mode = ?),0) THEN 'paid' ELSE 'refund_pending' END, paid_at = CASE WHEN amount > COALESCE((SELECT MAX(amount) FROM sponsor_bids WHERE paid_at IS NOT NULL AND test_mode = ?),0) THEN ? ELSE NULL END, payment_intent = ? WHERE id = ? AND status = 'checkout'").bind(bid.test_mode,bid.test_mode,Date.now(),session.payment_intent,bid.id).run();
...
export async function paymentStatus(db,env,id,player){const bid=await db.prepare('SELECT * FROM sponsor_bids WHERE id = ? AND player_id = ?').bind(id,player).first();if(!bid)fail('Payment not found in this browser.',404);if(bid.test_mode!==testMode(env))fail('This payment belongs to another environment.',409);if(bid.session_id&&['checkout','refund_pending'].includes(bid.status)){const session=await stripe(env,'checkout/sessions/'+encodeURIComponent(bid.session_id));await fulfillSession(db,env,session)}const result=await db.prepare('SELECT status FROM sponsor_bids WHERE id = ?').bind(id).first();return {...result,current:(await highBid(db,bid.test_mode))?.id===id}}
...
export async function ownerBids(db){const rows=await db.prepare('SELECT id, name, url, amount, status, hidden, test_mode AS testMode, session_id AS sessionId, paid_at AS paidAt FROM sponsor_bids ORDER BY created_at DESC LIMIT 100').all();return {bids:rows.results}}
