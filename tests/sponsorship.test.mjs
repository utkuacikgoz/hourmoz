...
globalThis.fetch = async (url, options) => {
  ...
  if (path === '/v1/checkout/sessions') {
    const id = params.get('metadata[bid_id]'),
      session = {
        id: 'cs_test_' + id,
...
try {
  ...
  for (const data of [
    {...bid(400)},
    {...bid(550)},
    {...bid(500), url: 'javascript:alert(1)'},
    {...bid(500), accepted: false},
    {...bid(500), name: '<script>'},
  ])
    await assert.rejects(() => submitBid(DB, env, data, player));
  const a = bid(500),
    b = bid(600),
    equal = bid(500);
  for (const data of [a, b, equal]) await submitBid(DB, env, data, player);
  ...
  assert.equal((await fulfillSession(DB, env, sessions.get(b.id))).status, 'paid');
  assert.equal((await paidSponsor(DB, 1)).id, b.id);
  failRefund = true;
  await assert.rejects(() => fulfillSession(DB, env, sessions.get(equal.id)), /unavailable/);
  assert.equal(status(equal.id), 'refund_pending');
  failRefund = false;
  assert.equal((await fulfillSession(DB, env, sessions.get(equal.id))).status, 'refunded');
  await fulfillSession(DB, env, sessions.get(equal.id));
  assert.equal(refunds.size, 1);
  assert.equal((await paidSponsor(DB, 1)).id, b.id);
  assert.equal((await publicAuction(DB, true)).bids.length, 0);
  assert.equal(await paidSponsor(DB), null);
  const summary = await publicAuction(DB, true, 1);
  assert.equal(summary.minimum, 700);
  assert.equal(summary.bids.length, 2);
  assert(!JSON.stringify(summary).includes(player));
  assert(!JSON.stringify(summary).includes('session_id'));
  const c = bid(700),
    d = bid(700);
  await submitBid(DB, env, c, player);
  await submitBid(DB, env, d, player);
  await Promise.all([fulfillSession(DB, env, sessions.get(c.id)), fulfillSession(DB, env, sessions.get(d.id))]);
  assert.equal(
    DB.sqlite.prepare("SELECT COUNT(*) AS count FROM sponsor_bids WHERE amount=700 AND status='paid'").get().count,
    1,
  );
  const raw = '{"type":"test"}',
  ...
  const earlyRefund = bid(800);
  await submitBid(DB, env, earlyRefund, player);
  ...
  assert.notEqual((await paidSponsor(DB, 1)).id, earlyRefund.id);
  console.log(
    'PASS: checkout validation, private pending bids, confirmed-payment activation, monotonic sponsor price, concurrent equal bids, idempotent delivery/refund retries, webhook signatures.',
  );

  // Refunded, disputed and hidden placements hand the spot back to the next active sponsor and the floor follows.
  const top = await paidSponsor(DB, 1);
  assert.equal(top.amount, 700);
  const e = bid(900);
  await submitBid(DB, env, e, player);
  assert.equal((await fulfillSession(DB, env, sessions.get(e.id))).status, 'paid');
  assert.equal((await paidSponsor(DB, 1)).id, e.id);
  assert.equal((await publicAuction(DB, true, 1)).minimum, 1000);
  await webhook({
    type: 'charge.refunded',
    data: {object: {refunded: true, payment_intent: sessions.get(e.id).payment_intent}},
  });
  assert.equal(status(e.id), 'refunded');
  assert.equal((await paidSponsor(DB, 1)).id, top.id);
  assert.equal((await publicAuction(DB, true, 1)).minimum, 800);
  const topIntent = sessions.get(top.id).payment_intent;
  await webhook({
    type: 'charge.dispute.created',
    data: {object: {status: 'needs_response', payment_intent: topIntent}},
  });
  assert.equal(status(top.id), 'disputed');
  assert.equal((await paidSponsor(DB, 1)).id, b.id);
  assert.equal((await publicAuction(DB, true, 1)).minimum, 700);
  await webhook({type: 'charge.dispute.closed', data: {object: {status: 'won', payment_intent: topIntent}}});
  assert.equal(status(top.id), 'paid');
  assert.equal((await paidSponsor(DB, 1)).id, top.id);
  await webhook({
    type: 'charge.dispute.created',
    data: {object: {status: 'needs_response', payment_intent: null, charge: topIntent.replace('pi_', 'ch_')}},
  });
  await webhook({type: 'charge.dispute.closed', data: {object: {status: 'lost', payment_intent: topIntent}}});
  assert.equal(status(top.id), 'refunded');
  assert.equal((await paidSponsor(DB, 1)).id, b.id);
  await reviewBid(DB, {id: b.id, action: 'hide'});
  assert.equal((await paidSponsor(DB, 1)).id, a.id);
  assert.equal((await publicAuction(DB, true, 1)).minimum, 600);
  await reviewBid(DB, {id: b.id, action: 'show'});
  assert.equal((await paidSponsor(DB, 1)).id, b.id);
  const f = bid(1000);
  await submitBid(DB, env, f, player);
  await webhook({type: 'checkout.session.expired', data: {object: {id: sessions.get(f.id).id}}});
  assert.equal(status(f.id), 'expired');
  await assert.rejects(() => submitBid(DB, env, f, player), /expired/);
  assert.equal((await fulfillSession(DB, env, sessions.get(f.id))).status, 'paid');
  assert.equal((await paidSponsor(DB, 1)).id, f.id);
  assert.equal((await ownerBids(DB)).bids[0].status, 'paid');
  assert(!(await ownerBids(DB)).bids.some(row => row.status === 'expired'));
  const ceiling = bid(1000000);
  await submitBid(DB, env, ceiling, player);
  assert.equal((await fulfillSession(DB, env, sessions.get(ceiling.id))).status, 'paid');
  await assert.rejects(() => submitBid(DB, env, bid(1000000), player), /maximum price/);
  console.log(
    'PASS: refunded, disputed and hidden sponsors hand the spot back, expired checkouts can still settle, admin list puts paid first, ceiling is explained.',
  );
  ...
  assert.equal(
    (
      await request('/api/auction/admin', null, {
  ...
  for (let i = 0; i < 2; i++) assert.equal((await request('/api/presence', {visitId}, {Cookie: cookie})).status, 200);
