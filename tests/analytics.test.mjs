...
try {
  config.bookingUrl = 'https://example.com/book';
  config.schedule = [
    {id: 'test-only', day: new Date().toISOString().slice(0, 10), name: 'Test sponsor', url: 'https://example.com'},
  ];
  assert.equal((await sponsorship()).sponsor.id, 'test-only');
  assert.equal((await sponsorship(Date.now() + 86400000)).sponsor, null);
  ...
  config.schedule[0].url = 'javascript:alert(1)';
...
console.log(
  'PASS: source privacy, visit deduplication, cookie ownership, replay-safe funnel, analytics opt-out, dated sponsors, event deduplication, link safety and retention.',
);
