...
const shared=localDB();const from=(ip,path='/api/leaderboard?mode=run',method='GET',secret=null)=>limitRequest(new Request('https://game.test'+path,{method,headers:{'CF-Connecting-IP':ip}}),shared,secret);
let rejected=0;for(let i=0;i<200;i++){try{await from('203.0.113.9')}catch(e){assert.equal(e.status,429);rejected++}}assert.equal(rejected,20);
...
for(let i=0;i<6;i++)await from('203.0.113.11','/api/auction/bids','POST');await assert.rejects(()=>from('203.0.113.11','/api/auction/bids','POST'),{status:429});await from('203.0.113.11');
const before=shared.sqlite.prepare('SELECT COUNT(*) AS count FROM rate_limits').get().count;await from('203.0.113.12',undefined,undefined,'keyed-secret');assert.equal(shared.sqlite.prepare('SELECT COUNT(*) AS count FROM rate_limits').get().count,before+1);
console.log('PASS: client caps are checked before the shared budget, bids are counted separately, keyed hashing works.');

// Every page and API response carries the security headers; HSTS only over HTTPS.
const page=await worker.fetch(new Request('https://game.test/'),{DB:db});assert.equal(page.status,200);assert.match(page.headers.get('Content-Security-Policy'),/frame-ancestors 'none'/);assert.match(page.headers.get('Content-Security-Policy'),/script-src 'self'/);assert.equal(page.headers.get('X-Frame-Options'),'DENY');assert.equal(page.headers.get('Strict-Transport-Security'),'max-age=31536000; includeSubDomains');
const api=await worker.fetch(new Request('https://game.test/api/leaderboard?mode=run'),{DB:db});assert.equal(api.status,200);assert.equal(api.headers.get('X-Frame-Options'),'DENY');assert.equal(api.headers.get('Cache-Control'),'no-store');
assert.equal(securityHeaders(new URL('http://127.0.0.1:4173/'))['Strict-Transport-Security'],undefined);
const login=await worker.fetch(new Request('https://game.test/owner-login?returnTo=https://evil.example'),{DB:db});assert.equal(login.status,302);assert.equal(login.headers.get('Location'),'/sponsor-admin.html');
