...
const request=new Request('https://game.example',{headers:{'oai-authenticated-user-id':'spoof','oai-authenticated-user-email':'owner@example.com','Cf-Access-Authenticated-User-Email':'owner@example.com','Cf-Access-Jwt-Assertion':'forged'}});
...
assert.equal(await isOwner(request,env,{access:{aud:'admin-app',getIdentity:async()=>{throw new Error('identity service down')}}}),false);
assert.equal(await isOwner(request,env,identity('admin-app','Owner@Example.com')),true);
console.log('PASS: forged identity headers denied, legacy provider setting ignored, Access audience and owner identity required.');
